...
function EditPage() {
  ...
  useEffect(() => {
    const getStudent = async () => {
      try {
        const { data } = await axios.get(`http://localhost:3000/sinh_vien/${id}`)
        setName(data.name || '')
        setAge(data.age?.toString() || '')
        setMonHoc(data.mon_hoc || '')
        setNganhHoc(data.nganh_hoc || 'FE')
      } catch (error) {
  ...
  const handleSubmit = async event => {
    ...
    try {
      await axios.put(`http://localhost:3000/sinh_vien/${id}`, {
        name: name.trim(),
        age: ageNumber,
        mon_hoc: monHoc.trim(),
        nganh_hoc: nganhHoc,
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-semibold mb-6">Cập nhật sinh viên</h1>
